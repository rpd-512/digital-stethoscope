...
from scipy.signal import butter, sosfiltfilt, stft, hilbert
from matplotlib.animation import FuncAnimation
import tensorflow as tf

model = tf.keras.models.load_model("cnn_lstm_heartbeat_classifier.keras")
...
def update(frame):
    ...
    val = float(pred[0][0])
    print(f"Heartbeat probability: {val:.3f}", end=" → ")
    if(val > 0.5):
        print("Heartbeat detected!")
    else:
        print("No heartbeat detected.")
    if(val >= 0.5):
        ax_env.set_title("Amplitude Envelope - Heartbeat Detected", color="green")
    else:
